...
function cumlelereDonustur(cumleler, seperator = ",") {
  return cumleler.map((item) => item.join(seperator));
}
...
var manav = [];
manav.push(...meyveler.concat(sebzeler));
// console.log(manav)
...
function emojileriDonustur(mesaj, emojiler) {
  for (let i = 0; i < Object.keys(emojiler).length; i++){
    mesaj=mesaj
      .replaceAll(Object.keys(emojiler)[i].toLowerCase(), Object.values(emojiler)[i]).replaceAll(Object.keys(emojiler)[i].toUpperCase(), Object.values(emojiler)[i]);
	}
  return mesaj;
}
